...
{
    /// <summary>Signs in through <c>POST /api/auth/guest-login</c> and returns the minted bearer.</summary>
    public static async Task<string> GetDemoTokenAsync(this HttpClient client)
    {
        ...
        var login = await response.Content.ReadFromJsonAsync<GuestLoginResponse>(JsonDefaults.Options);
